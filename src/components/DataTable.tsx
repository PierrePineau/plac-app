...
import { useReactTable, getCoreRowModel, getSortedRowModel, ColumnDef, SortingState, flexRender } from "@tanstack/react-table";
...
interface DataTableProps<T> {
	...
	renderCell?: (cellValue: any, column: ColumnDef<T>, row: T) => React.ReactNode;
...
export default function DataTable<T extends object>({ children, store, isLoading = false, columns, onRowSelectionChange, renderCell, onRowClick, enableSorting = true, enableRowSelection = true, ellipsisEnabled = true, enableSearch = true }: DataTableProps<T>) {
								...
								{enableRowSelection && (
									<th className="w-14">
										<label className="w-full h-12 m-auto cursor-pointer flex justify-center items-center">
											<input type="checkbox" checked={selectedRows.size === data.length} onChange={handleSelectAll} />
										</label>
									</th>
								)}
								{headerGroup.headers.map((header) => (
									<th key={header.id} className={`p-3 text-left ${header.column.getCanSort() ? "cursor-pointer" : ""}`} onClick={header.column.getToggleSortingHandler()}>
							...
							table.getRowModel().rows.map((row) => (
								<tr key={row.id} onClick={() => onRowClick?.(row.original)}>
									...
									{row.getVisibleCells().map((cell, index) => (
										<td key={cell.id} className="text-sm text-neutral-600 flex-row justify-between">
											{renderCell ? renderCell(cell.getValue(), cell.column.columnDef, row.original) : flexRender(cell.column.columnDef.cell, cell.getContext())}
											{index === row.getVisibleCells().length - 1 && ellipsisEnabled && (
												<button className="ml-2" onClick={(e) => e.stopPropagation()}>
													<EllipsisVertical />
												</button>
											)}
										</td>
